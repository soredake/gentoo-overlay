...
VULKAN_PATCHES_V="d103ba9ef214ae8ee315e05f5caee8f5ef904fd7"
VULKAN_PATCHES_DIR="${WORKDIR}/3.3-vulkan-fix-${VULKAN_PATCHES_V}"
D3D9_P="wine-d3d9-${PV}"
D3D9_DIR="${WORKDIR}/wine-d3d9-patches-${D3D9_P}"
GWP_V="20180120"
PATCHDIR="${WORKDIR}/gentoo-wine-patches"

DESCRIPTION="Free implementation of Windows(tm) on Unix, with optional external patchsets (with pba and full vulkan)"
...
if [[ ${PV} == "9999" ]] ; then
	...
	VULKAN_PATCHES_EGIT_REPO_URI="https://github.com/GloriousEggroll/3.3-vulkan-fix.git"
else
	SRC_URI="${SRC_URI}
	...
	https://github.com/GloriousEggroll/3.3-vulkan-fix/archive/${VULKAN_PATCHES_V}.tar.gz -> vulkan-patches-3.3.tar.gz"
fi
...
src_unpack() {
	if [[ ${PV} == "9999" ]] ; then
		...
		if use staging; then
			...
			git-r3_fetch "${VULKAN_PATCHES_EGIT_REPO_URI}" "${VULKAN_PATCHES_COMMIT}"
			gitt-r3_checkout "${VULKAN_PATCHES_EGIT_REPO_URI}" "${VULKAN_PATCHES_DIR}"
		fi
		if use d3d9; then
...
src_prepare() {
	...
	#eapply "${VULKAN_PATCHES_DIR}/vulkan_patch_remove"
	for i in "${VULKAN_PATCHES_DIR}/vulkan_patch_remove"/*.patch; do patch -Np1 -R < $i; done
	#eapply "${VULKAN_PATCHES_DIR}/vulkan_patch"
	for i in "${VULKAN_PATCHES_DIR}/vulkan_patch"/*.patch; do patch -Np1 < $i; done
