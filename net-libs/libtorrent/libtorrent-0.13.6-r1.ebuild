...
SRC_URI="
	...
	ps? (
		https://raw.githubusercontent.com/pyroscope/rtorrent-ps/master/patches/lt-base-cppunit-pkgconfig.patch
		https://raw.githubusercontent.com/pyroscope/rtorrent-ps/master/patches/lt-ps-honor_system_file_allocate_all.patch
		https://raw.githubusercontent.com/pyroscope/rtorrent-ps/master/patches/lt-ps-log_open_file-reopen_all.patch
	)"
...
IUSE="debug ipv6 libressl ssl test +ps"
...
src_prepare() {
	if use ps ; then
		list=(
			...
			"${DISTDIR}"/lt-ps-honor_system_file_allocate_all.patch
			"${DISTDIR}"/lt-ps-log_open_file-reopen_all.patch
		)
		eapply "${list[@]}"
	fi
	eapply_user
